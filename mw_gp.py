...
def single_predictor_mw_gp(
    ...
    Args:
        ...
        downsample_n (int): Default 2000. The size of the downsample, only used if
            downsample is set to True (True is the default behaviour).
...
def gp_model_only(
    ...
    # Initiate the kernel used for the GP
    initial_scale = np.ones(n_params)
    kernel = gp.kernels.RBF(length_scale=initial_scale) + gp.kernels.WhiteKernel(noise_level=1,
                                                                                 noise_level_bounds=(1e-5, 1e5))
    model = gp.GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10, normalize_y=True)

    # Sigma restriction for largest samples
    if restrict_sigma:
        temp_mw = mw.replace(0,
                             10000)  # this is used when running at a nominal value with no variance but still restricting sigma
        galaxies, temp_mw = galaxies.align(temp_mw, axis=1, copy=False)
        conditional = np.logical_and(
            galaxies < (temp_mw.loc["mean"] + sigma_cutoff * temp_mw.loc["sigma"]),
            galaxies > (temp_mw.loc["mean"] - sigma_cutoff * temp_mw.loc["sigma"]))
